...
const EQ_COLORS = {
  hpf: "#ef4444",
  lpf: "#2563eb",
  bands: ["#22c55e", "#38bdf8", "#a855f7", "#f59e0b", "#f97316", "#ec4899", "#14b8a6"],
};
const CHART_THEME = {
...
type EqSelection = "hpf" | "lpf" | number;
  ...
  return (
    ...
    >
      ...
      {horizontalMajorDb.map((gain) => (
        <g key={gain}>
          <line
            ...
            y1={yForGain(gain)}
            y2={yForGain(gain)}
            stroke={gain === 0 ? CHART_THEME.gridAccent : CHART_THEME.gridMajor}
            strokeWidth="1"
          />
          <text x={graphX - 14} y={yForGain(gain) + 4} fill={CHART_THEME.axisLabel} fontSize="9" textAnchor="end" fontWeight="500">
            {gain > 0 ? `+${gain}` : gain}
          </text>
        </g>
      ))}
      ...
      <polyline
        ...
        fill={CHART_THEME.curveFill}
        opacity={eq.enabled ? 0.13 : 0.05}
      />
      ...
      <polyline points={points} fill="none" stroke={CHART_THEME.curveStroke} strokeWidth="2.2" />
        ...
        return (
          ...
          >
            <line
              ...
              strokeDasharray="4 5"
              opacity={enabled ? 0.42 : 0.16}
            />
            {isSelected && <circle cx={x} cy={yForGain(0)} r={17} fill="none" stroke={color} strokeWidth="2" opacity="0.45" />}
            <circle cx={x} cy={yForGain(0)} r={12} fill={color} stroke={isSelected ? "#f8fafc" : "#020617"} strokeWidth="2" opacity={enabled ? 1 : 0.38} />
            <text x={x - 5} y={yForGain(0) + 5} fill="#fff" fontSize="12" fontWeight="900">
              {label}
            </text>
          </g>
        );
      ...
      {eq.bands.map((band, index) => {
        ...
        return (
          ...
          >
            {isSelected && <circle cx={x} cy={y} r={17} fill="none" stroke={color} strokeWidth="2" opacity="0.45" />}
            <circle cx={x} cy={y} r={12} fill={color} stroke={isSelected ? "#f8fafc" : "#020617"} strokeWidth="2" opacity={eq.enabled && band.enabled ? 0.95 : 0.32} />
            <text x={x - 4} y={y + 5} fill="#fff" fontSize="12" fontWeight="900">
              {index + 1}
            </text>
          </g>
        );
...
function EqEditor({
  ...
  const [selectedNode, setSelectedNode] = useState<EqSelection>(1);
